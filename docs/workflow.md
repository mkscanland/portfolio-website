...
4. **After finishing the task**, run these final checks from the repository root, in order:
   ...
   If any check fails, fix the issue and rerun the affected check. Report a check you could not run and why. Browser tests use the built application and cover responsive behavior that jsdom cannot measure.
...
Pull requests into `main` run build, unit tests, browser regression tests, and the nonmutating lint check. Deployment only runs on a successful push to `main`.
